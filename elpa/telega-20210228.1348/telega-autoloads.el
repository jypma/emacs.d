...
(defvar telega-prefix-map (let ((map (make-sparse-keymap))) (define-key map (kbd "t") 'telega) (define-key map (kbd "c") 'telega-chat-with) (define-key map (kbd "i") 'telega-switch-important-chat) (define-key map (kbd "s") 'telega-saved-messages) (define-key map (kbd "b") 'telega-switch-buffer) (define-key map (kbd "f") 'telega-buffer-file-send) (define-key map (kbd "w") 'telega-browse-url) (define-key map (kbd "a") 'telega-account-switch) map) "\
...
(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "telega-filter" '("all" "any" "archive" "can-get-statistics" "chat-list" "contact" "custom" "default-disable-notification" "define-telega-filter" "fake-or-scam" "folder" "has-" "ids" "inactive-supergroups" "last-message-by-me" "main" "my-permission" "name" "nearby" "not" "online-status" "permission" "pin" "replies-messages" "restriction" "saved-messages" "search" "top" "tracking" "type" "unmuted" "unread" "verified")))
